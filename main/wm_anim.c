...
void wm_animate_launch(lv_obj_t *src, lv_obj_t *target) {
  ...
  /* If the source has a label child (e.g. an icon symbol), mirror it. */
  uint32_t child_cnt = lv_obj_get_child_cnt(src);
  for (uint32_t i = 0; i < child_cnt; i++) {
    lv_obj_t *child = lv_obj_get_child(src, i);
    if (lv_obj_check_type(child, &lv_label_class)) {
      lv_obj_t *label = lv_label_create(ghost);
      lv_label_set_text(label, lv_label_get_text(child));
      lv_obj_set_style_text_color(label,
                                  lv_obj_get_style_text_color(child, LV_PART_MAIN),
                                  0);
      lv_obj_center(label);
      break;
    }
